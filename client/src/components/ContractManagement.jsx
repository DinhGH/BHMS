...
function ContractManagement() {
  ...
  const tenantOptionsByRoom = useMemo(() => {
    ...
    return tenants.filter((tenant) => String(tenant.roomId) === String(form.roomId));
  }, [tenants, form.roomId]);
  ...
  const roomOptionsByHouse = useMemo(() => {
    ...
    return rooms.filter((room) => String(room.houseId) === String(form.houseId));
  }, [rooms, form.houseId]);
    ...
    <div className="w-full">
                ...
                {loading ? (
                  ...
                  paginatedContracts.map((contract) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium">{contract.room?.name || "-"}</div>
                        <div className="text-xs text-gray-400">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium mb-1">Boarding House *</label>
            <select
          ...
          <div>
            ...
            >
              ...
              {tenantOptionsByRoom.map((tenant) => (
      ...
      <Modal
        open={openDetail}
        title={contractDetail ? `Contract Detail #${contractDetail.id}` : "Contract Detail"}
        onClose={() => {
          setOpenDetail(false);
      ...
      >
        {loadingDetail ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <p>
              <span className="font-semibold">Contract ID:</span> #{contractDetail.id}
            </p>
            <p>
              <span className="font-semibold">Status:</span> {contractDetail.status}
            </p>
            <p>
              <span className="font-semibold">Room ID:</span> {contractDetail.roomId}
            </p>
            <p>
              <span className="font-semibold">Tenant ID:</span> {contractDetail.tenantId}
            </p>
            <p>
              <span className="font-semibold">Room Name:</span> {contractDetail.room?.name || "-"}
            </p>
            <p>
      ...
      <Modal
        open={openInvoices}
        title={invoiceContract ? `Invoice History - Contract #${invoiceContract.id}` : "Invoice History"}
        onClose={() => {
          setOpenInvoices(false);
          setInvoiceHistory([]);
              ...
              <tbody>
                {invoiceHistory.map((invoice) => (
                  <tr key={invoice.id} className="border-b last:border-none">
                    ...
                    <td className="py-2 pr-3">{formatDate(invoice.createdAt)}</td>
                    <td className="py-2 pr-3">
                  ...
                  <table className="w-full min-w-190 text-sm">
                    ...
                    <tbody>
                      {stayHistory.tenantHistory.map((item) => (
                        <tr key={`tenant-${item.id}`} className="border-b last:border-none">
                          <td className="py-2 px-3">#{item.id}</td>
                          <td className="py-2 px-3">
                            {item.room?.name || "-"} ({item.room?.boardingHouseName || "-"})
                          </td>
                          <td className="py-2 px-3">{formatDate(item.startDate)}</td>
                          <td className="py-2 px-3">{formatDate(item.endDate)}</td>
                          <td className="py-2 px-3">
                            <span
                  ...
                  <table className="w-full min-w-190 text-sm">
                    ...
                    <tbody>
                      {stayHistory.roomHistory.map((item) => (
                        <tr key={`room-${item.id}`} className="border-b last:border-none">
                          <td className="py-2 px-3">#{item.id}</td>
                          <td className="py-2 px-3">
                            {item.tenant?.fullName || "-"} ({item.tenant?.email || "-"})
                          </td>
                          <td className="py-2 px-3">{formatDate(item.startDate)}</td>
                          <td className="py-2 px-3">{formatDate(item.endDate)}</td>
                          <td className="py-2 px-3">
